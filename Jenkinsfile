pipeline {
  ...
  sh 'ssh -t -t ec2-user@44.234.190.91 -o StrictHostKeyChecking=no "minikube start && kubectl delete pods capstone && kubectl run capstone --image=yaraselim1994/cloud-devops-capstone --port=80 && sleep 30s && kubectl get pods && kubectl port-forward --address 0.0.0.0 capstone 8888:80 &"'
}
            }
        }
    }
